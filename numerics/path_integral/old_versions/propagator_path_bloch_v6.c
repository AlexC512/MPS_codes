...
void CREATE_INDEX_FROM_PATH_VECTOR(int *path_vector, int MAX_POWER, int *pow4)
{
...
CREATE_DOUBLE_PATH_VECTOR(PATH,1000000,560,MAX_POWER,pow4); // we can only create a path in two steps, first step
printf("(%i",PATH[0]);  for (i=1;i<Nc;i++) printf(",%i",PATH[i]); printf(")\n"); 

PATH[16]=1;
printf("(%i",PATH[0]);  for (i=1;i<Nc;i++) printf(",%i",PATH[i]); printf(")\n"); 
// checking reverse mapping works
CREATE_INDEX_FROM_PATH_VECTOR(PATH,MAX_POWER,pow4);
printf("(%i,%i) \n",PATH[0],PATH[1]);
// ###########################################################################
...
int *REL_STATES_INDEX   = calloc( 2*(655360+1), sizeof(int));
int *REL_STATES_INDEX_2 = calloc( 2*(655360+1), sizeof(int));

// first calculate the first path half 
//for(p=1;p<Nc;p++){
// expectation values per run

// ====================================
// calculate the first 10 steps at once
// STEP 10
// ====================================
...
for (k=0;k<pow4[10];k++){	
 ...
 if ( ((k+l)%1000000 == 0) ) {
     printf("(%i",PATH[0]);  for (i=1;i<Nc;i++) printf(",%i",PATH[i]); printf(") -- %i \n",NEGLIGIBLE); 
  }
 for(i=0;i<p+1;i++)
 { 
 ...
 if ( ( cabs(test) < 0.00000001 ) )     NEGLIGIBLE++;
 else {
        if (PATH[p]==3) rho11 += test;
        if (PATH[p]==0) rho00 += test;
        if (PATH[p]==1) rho01 += test;
        if (PATH[p]==2) rho10 += test;
        rs_index(NON_NEGLIGIBLE,0)=k;
        rs_index(NON_NEGLIGIBLE,1)=l;
        NON_NEGLIGIBLE++;
      }
}  //l loop
} // k loop
...
printf("NON_NEGLIGIBLE %i \n",NON_NEGLIGIBLE);

printf("(%i",PATH[0]);  for (i=1;i<Nc;i++) printf(",%i",PATH[i]); printf(") -- %i \n",NEGLIGIBLE);

int MAX_NN = NON_NEGLIGIBLE;

// ------------------------------------------------------------
// now calculate the next step with reduced paths array 
// STEP 11
// ------------------------------------------------------------
rho11=0.;rho10=0.;rho01=0.;rho00=0.;
MAX_NN = NON_NEGLIGIBLE; NON_NEGLIGIBLE=0;

p=11; // until 8 l=0, 9 l=1
for(l=0;l<4;l++)
{ 
for(k=0;k<MAX_NN;k++)
{	
 ...
 if ( ( cabs(test) < 0.00000001 ) )     NEGLIGIBLE++;
        ...
        rs_2_index(NON_NEGLIGIBLE,0)=rs_index(k,0); // first index stays
        rs_2_index(NON_NEGLIGIBLE,1)=rs_index(k,1) + l*pow4[p-9]; // second index belonging to path 
...
{	
 ...
 if ( ( cabs(test) < 0.00000001 ) )     NEGLIGIBLE++;
        ...
        rs_index(NON_NEGLIGIBLE,0)=rs_2_index(k,0); // first index stays
        rs_index(NON_NEGLIGIBLE,1)=rs_2_index(k,1) + l*pow4[p-9]; // second index belonging to path 
...
// Now I calculated STEP 11 with reduced paths
// Check whether I can calculated now 
// STEP 13  -- FIRST write new reduced path set into old array
// -------------------------------------------
MAX_NN = NON_NEGLIGIBLE; NON_NEGLIGIBLE=0;
//for(k=0;k<MAX_NN;k++) 	

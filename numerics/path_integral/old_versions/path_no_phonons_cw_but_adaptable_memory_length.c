 ...
 ***************************************************/
 ...
 #define CUT_OFF_DENSITY ( 0.00000001 * 5.333 * 1.  )
 #define N_DELTA_T ( 70 )
...
// ############ End Phonon Kernel ############################
// ###########################################################
int MAX_NUMBER_OF_STATES = 4194304;
...
//printf("[%i]:t=%.5fps: RE[01]=%.10f -- IBM  =%.10f -- DIFF=%.10f \n",p,p*delta_t*0.001,creal(RESULT[1]),creal(IBM[p-1]),creal(IBM[p-1])-cimag(RESULT[1]) );
...
printf("[%i]:t=%.5fps: PULSE =%.10f --  \n\n",m,m*delta_t*0.001,creal(PULSE[m-1]));
...
fprintf(f_dat,"%.20f  \t %.20f \t",DYNAMICS_01[k],DYNAMICS_11[k]);

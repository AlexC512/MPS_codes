 ...
 ***************************************************/
 ...
 #define CUT_OFF_DENSITY ( 0.00000001 * 10.0333 * 0.  )
...
complex double init_01=AW_01;   
complex double init_10=AW_10;   
...
printf("[%i]:t=%.5fps: ADM_01=%.10f -- BLOCH=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(rho01),0.5*sin(2.*PULSE_AREA[m]),cimag(rho01)-0.5*sin(2.*PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: ADM_01=%.10f -- IBM  =%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(rho01),creal(IBM[m]),creal(IBM[m])-cimag(rho01) );
...
{	
 ...
 for(i=0;i<p;i++)
 {
  ...
  // include the initial state of the adm
    ...
    for(m=0;m<=i;m++) {Snm *= cexp( ( (PATH[i]/2) - (PATH[i]%2) ) * ( (PATH[m]/2) - (PATH[m]%2) ) * PHONON_KERNEL[i-m]); }	
 }
 
 if ( ( cabs(test) > CUT_OFF_DENSITY ) )   
    {
        if (PATH[p]==0) rho00 += test*Snm;
        if (PATH[p]==1) rho01 += test*Snm;
        if (PATH[p]==2) rho10 += test*Snm;
		if (PATH[p]==3) rho11 += test*Snm;
...
m=Dt+21;
///*
printf("[%i]:t=%.5fps: ADM_00=%.15f -- EXACT=%.15f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(rho00),cos(PULSE_AREA[m])*cos(PULSE_AREA[m]),creal(rho00)-cos(PULSE_AREA[m])*cos(PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: ADM_01=%.15f -- BLOCH=%.15f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(rho01),0.5*sin(2.*PULSE_AREA[m]),cimag(rho01)-0.5*sin(2.*PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: ADM_01=%.15f -- IBM  =%.15f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(rho01),creal(IBM[m]),creal(IBM[m])-cimag(rho01) );
printf("[%i]:t=%.5fps: ADM_11=%.15f -- EXACT=%.15f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(rho11),sin(PULSE_AREA[m])*sin(PULSE_AREA[m]),creal(rho11)-sin(PULSE_AREA[m])*sin(PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: PULSE =%.15f \n\n",m,m*delta_t*0.001,creal(PULSE[m]) );
printf("---- RELEVANT STATES: %i --- \n",NON_NEGLIGIBLE);
//*/
// ###########################################################################
// ###########################################################################

 ...
 ***************************************************/
 ...
 #define t_NULL   ( delta_t * 15  )
 #define WIDTH    ( 1.05*1.*7000.) // FULL WIDTH AT HALF MAXIMUM .. paper
...
#define M_Om(m,n,i)  ( *(Hsys    + 0 + 4*(i) + 2*(m) + (n) )) 
...
double OmR,ts;
complex double int_puls =0.;
int tp;
complex double *Hsys        = calloc(2*4*N,sizeof(double));
...
for(ts=0;ts<20;ts++)
   ...
   int_puls += (delta_t/Nc) * (5.0*OmL/WIDTH)*cexp(-1.0*( tp-t_NULL)*(tp-t_NULL)*1.386/(WIDTH*WIDTH)); 
   }
...
M_Om(0,0,k) =    cos(OmR);
M_Om(1,1,k) =    cos(OmR);
M_Om(0,1,k) = -I*sin(OmR);
M_Om(1,0,k) = -I*sin(OmR);  
}   

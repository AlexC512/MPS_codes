...
//OmR = delta_t*OmL;
M_Om(0,0,k) =    cos(OmR*delta_t*0.);
M_Om(1,1,k) =    cos(OmR*delta_t*0.);
M_Om(0,1,k) = -I*sin(OmR*delta_t*0.);
M_Om(1,0,k) = -I*sin(OmR*delta_t*0.);  
}   
...
printf("[%i]:t=%.5fps: ADM_01=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",p,p*delta_t*0.001,cimag(rho01),creal(IBM[p]),cimag(rho01)-
creal(IBM[p]) );

printf("[%i]:t=%.5fps: ADM_10=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",p,p*delta_t*0.001,cimag(rho10),-creal(IBM[p]),cimag(rho10)+creal(IBM[p]) );
...
printf("[%i]:t=%.5fps: ADM_01=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(rho01),creal(IBM[m]),cimag(rho01)-
creal(IBM[m]) );

printf("[%i]:t=%.5fps: ADM_10=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(rho10),-creal(IBM[m]),cimag(rho10)+creal(IBM[m]) );
...
snprintf(FILE_NAME,2048+1024,"Bloch_Path_Integral_t=%.1fps_CutOff=%.5f_Nc=%i_DETUNING=%.6f_OmL=%.5f_%.0fK.dat",m*delta_t*0.001,CUT_OFF_DENSITY/GOmL,Nc,DELTA/GOmL,GOmL,TEMPERATUR); 

FILE *f_dat; f_dat = fopen(FILE_NAME,"w");
for(k=0;k<21+Dt;k++)
fprintf(f_dat,"%.5f \t %.10f \t %.10f \n",delta_t*k*0.001*0.001,DYNAMICS_01[k],creal(IBM[k]) );
fclose(f_dat); 

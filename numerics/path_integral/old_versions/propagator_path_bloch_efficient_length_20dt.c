...
int *REL_STATES_INDEX   = calloc( 2*(1048576+1), sizeof(int));
int *REL_STATES_INDEX_2 = calloc( 2*(1048576+1), sizeof(int));
...
for (k=0;k<pow4[10];k++){	
 ...
 if ( ( cabs(test) < 0.00000001 ) )     NEGLIGIBLE++;
        ...
        rs_2_index(NON_NEGLIGIBLE,0)=k;
        rs_2_index(NON_NEGLIGIBLE,1)=l;
        NON_NEGLIGIBLE++;
      }
}  //l loop
} // k loop
...
printf("p=%i:t=%.5fps: ADM_00=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",p,OmRp/OmR*0.001,creal(rho_past(p-1,0)),cos(OmRp)*cos(OmRp),creal(rho_past(p-1,0) )-cos(OmRp)*cos(OmRp) );
printf("p=%i:t=%.5fps: ADM_01=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",p,OmRp/OmR*0.001,cimag(rho_past(p-1,1) ),sin(OmRp)*cos(OmRp),cimag(rho_past(p-1,1) )-sin(OmRp)*cos(OmRp) );
printf("p=%i:t=%.5fps: ADM_10=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",p,OmRp/OmR*0.001,cimag(rho_past(p-1,2) ),-sin(OmRp)*cos(OmRp),cimag(rho_past(p-1,2))+sin(OmRp)*cos(OmRp) );
printf("p=%i:t=%.5fps: ADM_11=%.10f -- EXACT=%.10f -- DIFF=%.10f \n\n",p,OmRp/OmR*0.001,creal(rho_past(p-1,3)),sin(OmRp)*sin(OmRp),creal(rho_past(p-1,3))-sin(OmRp)*sin(OmRp) );

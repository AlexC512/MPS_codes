 ...
 ***************************************************/
 ...
 #define GOmL      ( 1.284 * 1.0  ) 
 #define OmL      ( 0.0000784 * 0.0  ) 
 #define DELTA    ( 0.0135*0. )
 #define t_NULL   ( delta_t * N*0.25 )
...
complex double *Hsys        = calloc(2*N*4,sizeof(double));
double int_puls,tp;
int ts;
...
double DYNAMICS_00[N];
double DYNAMICS_11[N];
...
// ###########################################################################
// ###########################################################################
DYNAMICS_00[Dt+21]=creal(rho00);
DYNAMICS_11[Dt+21]=creal(rho11);
} // Dt loop

FILE *f_dat; f_dat = fopen("BLOCH_DYNAMICS_GAUSSIAN.dat","w");
for(k=0;k<21+Dt;k++)
fprintf(f_dat,"%.5f \t %.10f \t %.10f \t %.10f \t %.10f \n",delta_t*k*0.001,DYNAMICS_00[k],DYNAMICS_11[k],cos(OmR*delta_t)*cos(OmR*delta_t),sin(OmR*delta_t)*sin(OmR*delta_t) );
fclose(f_dat); 

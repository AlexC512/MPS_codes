...
complex double *adm      = calloc( 2*pow4[Nc]*(Nc-1) , sizeof(double) ); 
...
int D=1; int lpast,lnow,rpast,rnow,p; 
 ...
 { 
  ...
  if (i==0)   
       { 
        ...
        rho(p-1,k) *= M_Om(lnow,lpast) * init_rho * Md_Om(rpast,rnow);  
       }

 else rho(p-1,k) *= M_Om(lnow,lpast) * Md_Om(rpast,rnow);
      
 }
 
 // take the expectation values
 if (PATH[p]==3) rho11 += rho(p-1,k);
 if (PATH[p]==0) rho00 += rho(p-1,k);
 if (PATH[p]==1) rho01 += rho(p-1,k);
 if (PATH[p]==2) rho10 += rho(p-1,k);
...
for(p=1;p<Nc;p++)
{    
 // initialize for every p run (which is a time step) the expectation values   
 rho11=0.;rho10=0.;rho01=0.;rho00=0.;
   
for (k=0;k<pow4[11];k++)
{	
 D=10; // in the beginning the path depth increases
 rho(p-1,k) = 1.;
 CREATE_PATH_VECTOR(PATH,k,Nc,pow4);
 for(i=0;i<D;i++)
 { 
 ...
 // calculate the expectation values for the new initial states 
 if (PATH[10]==3) rho11 += rho(p-1,k);
 if (PATH[10]==0) rho00 += rho(p-1,k);
 if (PATH[10]==1) rho01 += rho(p-1,k);
 if (PATH[10]==2) rho10 += rho(p-1,k);    
...
} // end of k loop -- possible path 
// expectation value take trace

// new initial states 
rho_past(p-1,0) = rho00; rho_past(p-1,1) = rho01; rho_past(p-1,2) = rho10; rho_past(p-1,3) = rho11;
// check with exact result (only bloch rotations)
OmRp = (20+p)*delta_t*OmR;
printf("t=%.5fps: ADM_00=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",OmRp/OmR*0.001,creal(rho_past(p-1,0)),cos(OmRp)*cos(OmRp),creal(rho_past(p-1,0) )-cos(OmRp)*cos(OmRp) );
printf("t=%.5fps: ADM_01=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",OmRp/OmR*0.001,cimag(rho_past(p-1,1) ),sin(OmRp)*cos(OmRp),cimag(rho_past(p-1,1) )-sin(OmRp)*cos(OmRp) );
printf("t=%.5fps: ADM_10=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",OmRp/OmR*0.001,cimag(rho_past(p-1,2) ),-sin(OmRp)*cos(OmRp),cimag(rho_past(p-1,2))+sin(OmRp)*cos(OmRp) );
printf("t=%.5fps: ADM_11=%.10f -- EXACT=%.10f -- DIFF=%.10f \n\n",OmRp/OmR*0.001,creal(rho_past(p-1,3)),sin(OmRp)*sin(OmRp),creal(rho_past(p-1,3))-sin(OmRp)*sin(OmRp) );
   
} // end of p loop (time increment)

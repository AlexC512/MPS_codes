 ...
 ***************************************************/
 ...
 #define CUT_OFF_DENSITY ( 0.00000001 * 150.05333 * 1.  )
 #define N_DELTA_T ( 100 )
...
for(p=1;p<5;p++){	
rho11=0.;rho10=0.;rho01=0.;rho00=0.; re_pol = 0.;
 ...
 for(i=0;i<p;i++)
 {
  ...
  // include the initial state of the adm
    ...
    { test *= cexp( ( (PATH[i]/2) - (PATH[i]%2) ) * ( (PATH[m]/2) - (PATH[m]%2) ) * PHONON_KERNEL[i-m]); 
      Snm  += ( (PATH[i]/2) - (PATH[i]%2) ) * ( (PATH[m]/2) - (PATH[m]%2) ) * PHONON_KERNEL[i-m];         }	
 }
 
 if ( ( cabs(test) > CUT_OFF_DENSITY ) )   
    {
        if (PATH[p]==0)   rho00 += test;
        if (PATH[p]==1)  {rho01 += test; re_pol += creal(Snm);   }
...
printf("[%i]:t=%.5fps: RE[01]=%.10f -- IBM  =%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(rho01),creal(IBM[m]),creal(IBM[m])-cimag(rho01) );
...
printf("[%i]:t=%.5fps: PULSE =%.10f -- Re01 =%.10f \n\n",m,m*delta_t*0.001,creal(PULSE[m]),re_pol*0.001);

//PRINT_PATH(PATH,Nc);
DYNAMICS_01[p]=cimag(rho01);
DYNAMICS_11[p]=creal(rho11);
     RE_POL[p]=creal(rho01);
}
...
printf("[%i]:t=%.5fps: RE[01]=%.10f -- IBM  =%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(rho01),creal(IBM[m]),creal(IBM[m])-cimag(rho01) );
printf("[%i]:t=%.5fps: ADM_11=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(rho11),sin(PULSE_AREA[m])*sin(PULSE_AREA[m]),creal(rho11)-sin(PULSE_AREA[m])*sin(PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: PULSE =%.10f -- Re01 =%.10f \n\n",m,m*delta_t*0.001,creal(PULSE[m]),0.001*re_pol);
//PRINT_PATH(PATH,Nc);
DYNAMICS_01[p]=cimag(rho01);
DYNAMICS_11[p]=creal(rho11);
     RE_POL[p]=creal(rho01);
...
if (Dt>0)  PHONON_KERNEL[20] -= kernel_temp;     
...
printf("[%i]:t=%.5fps: IM[01]=%.15f -- BLOCH=%.15f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(rho01),0.5*sin(2.*PULSE_AREA[m]),cimag(rho01)-0.5*sin(2.*PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: RE[01]=%.15f -- IBM  =%.15f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(rho01),creal(IBM[m]),creal(IBM[m])-cimag(rho01) );
...
// ###########################################################################
// ###########################################################################
DYNAMICS_01[m]=cimag(rho01);
DYNAMICS_11[m]=creal(rho11);
     RE_POL[m]=creal(rho01);
} // Dt loop
#endif

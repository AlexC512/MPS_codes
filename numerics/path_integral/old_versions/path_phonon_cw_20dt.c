 ...
 ***************************************************/
 ...
 #define T_STEPS 50
 #define CUT_OFF_DENSITY ( 0.00000001 * 50.1  )
 #define N_DELTA_T ( 50 )
 ...
 #define TEMPERATUR (20.0001)//(2.01)//(1.7500100001) 
...
// It seems, I need to fix delta_t to cover 3ps for the finite memory kernel of the 
// LA phonons ... so delta_t is fixed, as long, as I keep Nc fixed ...
// the result is not changed whether I include the influence on the phonons on a density matrix t-Nc delta_t,
// the phonon contribution is negligible ... so I calculate Nc time steps with phonons contributions, and then 

//!! TODO ... make larger delta_t possible for the laser-interaction!
...
//OmR = delta_t*OmL;
M_Om(0,0,k) =    cos(OmR*delta_t*1.);
M_Om(1,1,k) =    cos(OmR*delta_t*1.);
M_Om(0,1,k) = -I*sin(OmR*delta_t*1.);
M_Om(1,0,k) = -I*sin(OmR*delta_t*1.);  
}   
...
complex double init_00=1.0;   
complex double init_01=0.0;   
complex double init_10=0.0;   
complex double init_11=0.0;   
...
//if (Dt==1) { PHONON_KERNEL[1] += PHONON_KERNEL[0] ; PHONON_KERNEL[0] = 0.; }   
//if (Dt==2) { PHONON_KERNEL[2] += PHONON_KERNEL[1] ; PHONON_KERNEL[1] = 0.; }   
//if (Dt==3) { PHONON_KERNEL[3] += PHONON_KERNEL[2] ; PHONON_KERNEL[2] = 0.; }    

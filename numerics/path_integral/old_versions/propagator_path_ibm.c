...
{	// for every k I need a corresponding path
	...
	//check the mapping
	/*if (kl%4096==0){ printf("%*i ---------------------------------\n",3,npath);
	printf("%*i:(%i",4,kl,PATH1[0]); for (m=1;m<Nc;m++) printf(",%i",PATH1[m]); printf(")\n");	} 	// */
    }
...
// augmented density matrix with maximal path numbers of memory depth 14
complex double *adm = calloc( 2.*(pow4[Nc-1]-1)*(Nc+1), sizeof(double) ); 

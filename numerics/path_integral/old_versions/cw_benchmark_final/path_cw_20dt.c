 ...
 ***************************************************/
 ...
 #define TIME_DYNAMICS_TO_NS 0.010000
 #define T_STEPS 250
 #define CUT_OFF_DENSITY ( 0.00000001 * 5.1  )
 #define N_DELTA_T ( 5000 )
 ...
 #define GOmL      ( 0.002184 * 0.0  ) 
 #define OmL      ( 0.0000984 * 1.0  ) 
 #define DELTA    ( 0.0135*0. )
 #define t_NULL   ( delta_t *0.0 )
 #define WIDTH    ( 1.05*1.*1.0) // FULL WIDTH AT HALF MAXIMUM .. paper
...
if (k==0) PULSE_AREA[k] = 0.;
else PULSE_AREA[k] = delta_t * OmL + PULSE_AREA[k-1];
M_Om(0,0,k) =    cos(OmL*delta_t);
M_Om(1,1,k) =    cos(OmL*delta_t);
M_Om(0,1,k) = -I*sin(OmL*delta_t);
M_Om(1,0,k) = -I*sin(OmL*delta_t);  
}   
...
for(k=0;k<21+Dt;k++)
fprintf(f_dat,"%.5f \t %.10f \t %.10f \t %.10f \t %.10f \t %.10f \n",delta_t*k*0.001,PULSE[k],DYNAMICS_11[k],sin(PULSE_AREA[k])*sin(PULSE_AREA[k]),DYNAMICS_01[k],0.5*sin(2.*PULSE_AREA[k]));
fclose(f_dat); 

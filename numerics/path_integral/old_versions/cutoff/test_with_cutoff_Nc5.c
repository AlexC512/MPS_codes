 ...
 ***************************************************/
 ...
 #define CUT_OFF_DENSITY ( 0.00000001 * 1.10 * 0.  )
...
#define init(n,p)   ( *( INITIAL_STATES   + (n)*4 + (p)   ) )
#define init_2(n,p) ( *( INITIAL_STATES_2 + (n)*4 + (p)   ) )
...
complex double *Hsys        = calloc(2*(N_DELTA_T+2*Nc+4)*4, sizeof(double));
...
complex double *INITIAL_STATES = calloc( 4*2*MAX_NUMBER_OF_STATES, sizeof(double));
complex double *INITIAL_STATES_2 = calloc( 4*2*MAX_NUMBER_OF_STATES, sizeof(double));
...
{ 
 ...
 PATH[p] = l ;  // for Dt>0, we have  
 
 test = 1.; Snm = 1.;
 for(i=1;i<=p;i++) // p maximal Nc-1, now 4
 {    
    test *= M_Om((PATH[i]/2),(PATH[i-1]/2),i) * conj(M_Om((PATH[i]%2),(PATH[i-1]%2),i));
	for(m=0;m<i;m++) 
	{ Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m])) ); 
      if ( m>DEPTH ) DEPTH=m; 
	} //*/
    if (i==1) { test *= PATH_INIT[PATH[0]]; init_rho = test; }   
 }
 ...
 if ( ( cabs(test) > CUT_OFF_DENSITY ) )   
    {
		 ...
		 if (HELPER == 0) { 
		                   for(m=0;m<Nc;m++)    path_2(NON_NEGLIGIBLE,m)=PATH[m];
                     	   if (p==Nc-1) INITIAL_STATES[NON_NEGLIGIBLE]  = init_rho; 
		                  }
...
// if I calculate the next time step with the given path set - the trotter error increases, as I do not 
// calculate the next time step from an exact initial state! but in a non-Markovian problem, I cannot calculate a new reduced initial state
...
{	
 // first write path array   
 if (HELPER==0) { for(m=0;m<Nc-1;m++) PATH[m]=  path(k,m+1); }
 else           { for(m=0;m<Nc-1;m++) PATH[m]=path_2(k,m+1); }
 // now calculate the next time step 
 PATH[Nc-1] = l ;  // for Dt>0, we have   
 test = 1.; Snm = 1.;
 for(i=1;i<=Nc-1;i++) // p maximal Nc-1, now 4
 {    
    test *= M_Om((PATH[i]/2),(PATH[i-1]/2),i) * conj(M_Om((PATH[i]%2),(PATH[i-1]%2),i));
	/*for(m=0;m<i;m++) 
	{ Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m])) ); 
	...
	if (i==1)  { test *= INITIAL_STATES[k]; init_rho = test; }   
 }
 ...
 if ( ( cabs(test) > CUT_OFF_DENSITY ) )   
    {    
         ...
         // store the path 
         //printf("PATH[%i]:( ",NON_NEGLIGIBLE-1); for(m=0;m<Nc;m++) printf("%i,",path(k,m)); printf(")\n");
		 if (HELPER == 0) {for(m=0;m<Nc;m++) path_2(NON_NEGLIGIBLE,m)=PATH[m]; INITIAL_STATES_2[NON_NEGLIGIBLE]= init_rho;}
		 else             {for(m=0;m<Nc;m++)   path(NON_NEGLIGIBLE,m)=PATH[m];   INITIAL_STATES[NON_NEGLIGIBLE]= init_rho;}
         //printf("PATH[%i]:( ",NON_NEGLIGIBLE); for(m=0;m<Nc;m++) printf("%i,",path_2(NON_NEGLIGIBLE,m)); printf(")\n");
         NON_NEGLIGIBLE++;
     }
} // l loop four states 0,1,2,3 for new time step

 ...
 ***************************************************/
 ...
 #define CUT_OFF_DENSITY ( 0.00000001 * 0.10 * 1.  )
 ...
 if ( ( cabs(test) > CUT_OFF_DENSITY ) )   
    {
		 ...
		 Snm =  cexp( ((PATH[0]/2) - (PATH[0]%2)) * ((PATH[0]/2)*PHONON_KERNEL[0]-(PATH[0]%2)*conj(PHONON_KERNEL[0])) )
			   ...
			   *cexp( ((PATH[5]/2) - (PATH[5]%2)) * ((PATH[0]/2)*PHONON_KERNEL[5]-(PATH[0]%2)*conj(PHONON_KERNEL[5])) ) 
			   ;          
...
// ####################################################
// ####################################################
// get rid of initial state or PATH[0]
Dt=1;

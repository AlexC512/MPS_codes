...
{	
 ...
 {    
	...
	if (i==4) {m=0; // path4 to path4
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  ); 
	           m=1; // path4 to path3
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  ); 
	           m=2; // path4 to path2
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  ); 
	           m=3; // path4 to path1 -- as memory kernel 4 is zero, this is the last step 
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  );
			   m=4; // path4 to path0 -- as memory kernel 4 is zero, this is the last step 
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  );
			   // PRINT_PATH(PATH,Nc); path exist from 0 to 5
	          }			  
	if (i==5) {m=0; // path4 to path4
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  ); 
	           m=1; // path4 to path3
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  ); 
	           m=2; // path4 to path2
	           ...
	           m=3; // path4 to path1 -- as memory kernel 4 is zero, this is the last step 
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  );
			   m=4; // path4 to path1 -- as memory kernel 4 is zero, this is the last step 
		       Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  );
			   //m=5; // path4 to path1 -- as memory kernel 4 is zero, this is the last step 
		       //Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m]))  );
			   //PRINT_PATH(PATH,Nc); path exist from 0 to 4 
	          }			  
	}//Dt=0
	if (Dt>0)
	{

...
//PHONON_KERNEL[p] = 0. +I*0.;
if (p<10) printf("PHONON_KERNEL[%i]=%.15f+I%.10f -- IBM=%.10f \n",p,creal(PHONON_KERNEL[p]),cimag(PHONON_KERNEL[p]),creal(IBM[p]) );
}
// end IBM -------------------------------
// ---------------------------------------
...
{	
 ...
 {    
  ...
  // calculate the next system time step      
    test *= M_Om((PATH[i+1]/2),(PATH[i]/2),i+Dt) * conj(M_Om((PATH[i+1]%2),(PATH[i]%2),i+Dt));
	if (i<5) DEPTH= i; else DEPTH=5.;
    for(m=0;m<=DEPTH;m++) { Snm *= cexp( ((PATH[i+1]/2) - (PATH[i+1]%2)) * ((PATH[i+1-m]/2)*PHONON_KERNEL[m]-(PATH[i+1-m]%2)*conj(PHONON_KERNEL[m])) ); } 
 }
...
snprintf(FILE_NAME,2048+1024,"%d_Bloch_Phonon_wo_MEM_CUT_t=%.1fps_CutOff=%.5f_Nc=%i_DETUNING=%.6f_OmL=%.5f_GOmL=%.5f_%.0fK.dat",100*100*hour+100*min+sec,m*delta_t*0.001,CUT_OFF_DENSITY/(GOmL+OmL),Nc,DELTA/(GOmL+OmL),OmL,GOmL,TEMPERATUR); 
FILE *f_dat; f_dat = fopen(FILE_NAME,"w");
for(k=0;k<Nc+Dt-1;k++)
{
fprintf(f_dat,"%.5f \t",delta_t*k*0.001);
fprintf(f_dat,"%.10f \t %.10f \t %.10f \t",DYNAMICS_11[k],DYNAMICS_01[k],RE_POL[k]);

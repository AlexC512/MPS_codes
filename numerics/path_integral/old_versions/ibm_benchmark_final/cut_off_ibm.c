 ...
 ***************************************************/
 ...
 double delta_t = 500.00; // in fs //((double)TIME_DYNAMICS_TO_NS) * 1000. * 1000. /((double)T_STEPS); 
 #define CUT_OFF_DENSITY ( 0.00000001 * 1.333  )
 #define N_DELTA_T ( 4000 )
...
// #define PROGRESS_BEYOND_MEMORY
 ...
 #define AW_11 ( 0.0  )
 // x 1/fs --> 1000 x 1/ps 
 #define GOmL      ( 0.00075184 * 1.0  ) 
 #define OmL      ( 0.000001 * 1.0  ) 
 #define DELTA    ( 0.0135*0. )
 #define t_NULL   ( delta_t *10. )
 #define WIDTH    ( 1.05*1.*2500.0) 

 #define PHON_FAK (2.11 * 1.)
...
IBM[p] = 0.5*cexp(IBM_arg);
}

complex double dIBM; int max_mem=0;
for(p=0;p<Nc;p++)
{
	dIBM = 0.5;
	for(i=0;i<p;i++)
	{for(m=0;m<=i;m++) {dIBM *= cexp( PHONON_KERNEL[i-m] ); if ( (i-m)>max_mem ) max_mem = i-m;}  }
	printf("%i: num IBM=%.15f -- ana IBM=%.15f -- DIFF=%.15f \n",p,creal(dIBM),creal(IBM[p]),creal(dIBM-IBM[p]) );
}
printf("MAX_MEM=%i \n",max_mem);
...
// of course it works, if I keep the memory of the phonons but I don`t to exceed PHONON_KERNEL[20]
// now try to find the numerical cut-off... we start with a new initial condition
double kernel_temp = 0.;
double kernel_max  = creal(PHONON_KERNEL[20]);
// in the first time step beyond the cutoff only tk=20 exceeds the memory
kernel_temp = 0.;
for(i=0; i<N_b; i++)
      {
       float_q  = q_0 + delta_q * i;
	   ...
	   kernel_temp += spec_fak * ( cos(c_q*float_q*( 0. )*delta_t) - cos(c_q*float_q*(1.  )*delta_t)  ) ;    	   
      }
      printf("CORRECT=%.15f -- PHON_ZERO=%.15f \n",kernel_temp,creal(PHONON_KERNEL[0]));

    p=Nc+1; max_mem = 0;
	dIBM = creal(IBM[1]);
	PHONON_KERNEL[20] -= kernel_temp;
	for(i=0;i<Nc;i++)
	{for(m=0;m<=i;m++) {dIBM *= cexp( PHONON_KERNEL[i-m] ); if ( (i-m)>max_mem ) max_mem = i-m;}  }
	printf("%i: num IBM=%.15f -- ana IBM=%.15f -- DIFF=%.15f \n",p,creal(dIBM),creal(IBM[p]),creal(dIBM-IBM[p]) );
    printf("MAX_MEM=%i \n",max_mem);
...
for(i=0; i<N_b; i++)
      {
       ...
       spec_fak = -1. * delta_q * (1./(2.*M_PI*M_PI*c_q*c_q)) * la_coup[i]  * conj(la_coup[i]) * (2.*n_ph[i]+1.) ; 	
	   kernel_temp += spec_fak * ( cos(c_q*float_q* Nc     *delta_t) - cos(c_q*float_q*(Nc-1.)*delta_t)  ) ;  
	   kernel_temp += spec_fak * ( cos(c_q*float_q*( 1. )*delta_t) - cos(c_q*float_q*(2.  )*delta_t)  ) ;    	   
      }

    p=Nc+2; max_mem = 0;
	dIBM = creal(IBM[2]);
	PHONON_KERNEL[20] -= kernel_temp;
//	PHONON_KERNEL[19] -= PHONON_KERNEL[0];
	for(i=0;i<Nc;i++)
	{for(m=0;m<=i;m++) {dIBM *= cexp( PHONON_KERNEL[i-m] ); if ( (i-m)>max_mem ) max_mem = i-m;}  }
	printf("%i: num IBM=%.15f -- ana IBM=%.15f -- DIFF=%.15f \n",p,creal(dIBM),creal(IBM[p]),creal(dIBM-IBM[p]) );
    printf("MAX_MEM=%i \n",max_mem);

PHONON_KERNEL[20] = kernel_max;
for(p=1;p<1425;p++){
kernel_temp = 0.;
for(i=0; i<N_b; i++)
      {
       ...
       spec_fak = -1. * delta_q * (1./(2.*M_PI*M_PI*c_q*c_q)) * la_coup[i]  * conj(la_coup[i]) * (2.*n_ph[i]+1.) ; 	
	   kernel_temp += spec_fak * ( cos(c_q*float_q* Nc     *delta_t) - cos(c_q*float_q*(Nc-1.)*delta_t)  ) ;  
	   kernel_temp += spec_fak * ( cos(c_q*float_q*( p-1. )*delta_t) - cos(c_q*float_q*p      *delta_t)  ) ;    	   
      }
	...
	for(i=0;i<Nc;i++)
	{for(m=0;m<=i;m++) {dIBM *= cexp( PHONON_KERNEL[i-m] ); if ( (i-m)>max_mem ) max_mem = i-m;}  }
	printf("%i: num IBM=%.15f -- ana IBM=%.15f -- DIFF=%.15f \n",p,creal(dIBM),creal(IBM[p+Nc]),creal(dIBM-IBM[p+Nc]) );
    printf("MAX_MEM=%i \n",max_mem);
...
complex double init_00=AW_00;   
complex double init_01= I*0.5*sin(2.*PULSE_AREA[0]);//AW_01;   
complex double init_10=-I*0.5*sin(2.*PULSE_AREA[0]);//AW_01;   
complex double init_11=AW_11;   

DYNAMICS_01[0]=cimag(init_01);
DYNAMICS_11[0]=creal(init_11);
...
{	
 ...
 for(i=0;i<p;i++)
 {
  ...
  // include the initial state of the adm
    if (i==0)   
       { 
        if ( PATH[i]==3 ) init_rho  = init_11;
        if ( PATH[i]==1 ) init_rho  = init_01;
        if ( PATH[i]==2 ) init_rho  = init_10;
        if ( PATH[i]==0 ) init_rho  = init_00;
    ...
    for(m=0;m<=i;m++)
    test *= cexp( ( (PATH[i]/2) - (PATH[i]%2) ) * ( (PATH[m]/2) - (PATH[m]%2) ) * PHONON_KERNEL[i-m]);	
 }
...
printf("[%i]:t=%.5fps: PULSE =%.10f \n\n",m,m*delta_t*0.001,creal(PULSE[m]) );
...
{	
 ...
 for(i=0;i<p;i++)
 {
  ...
  // include the initial state of the adm
    ...
    // phonon contributions // PATH[i]%2 --> rpast, or rnow // PATH[i]/2 --> lpast,lnow
    for(m=0;m<=i;m++)
    test *= cexp( ( (PATH[i]/2) - (PATH[i]%2) ) * ( (PATH[m]/2) - (PATH[m]%2) ) * PHONON_KERNEL[i-m] );	
 }
...
printf("[%i]:t=%.5fps: ADM_00=%.10f -- EXACT=%.10f -- DIFF=%.10f\n",m,m*delta_t*0.001,creal(rho00),cos(PULSE_AREA[m])*cos(PULSE_AREA[m]),creal(rho00)-cos(PULSE_AREA[m])*cos(PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: ADM_01=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(rho01),0.5*sin(2.*PULSE_AREA[m]),cimag(rho01)-0.5*sin(2.*PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: ADM_01=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(rho01),0.5*sin(2.*PULSE_AREA[m]),cimag(rho01)-0.5*sin(2.*PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: ADM_11=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(rho11),sin(PULSE_AREA[m])*sin(PULSE_AREA[m]),creal(rho11)-sin(PULSE_AREA[m])*sin(PULSE_AREA[m]) );
printf("[%i]:t=%.5fps: PULSE =%.10f \n\n",m,m*delta_t*0.001,creal(PULSE[m]) );
//PRINT_PATH(PATH,Nc);
DYNAMICS_01[p]=cimag(rho01);
...
PHONON_KERNEL[20] -= PHONON_KERNEL[0];     
...
{	
 ...
 for(i=0;i<p;i++)
 {
  ...
  // include the initial state of the adm
    ...
    // phonon contribution ... but with cutoff!! // if p=5
    for(m=0;m<=i;m++) test *= cexp( ( (PATH[i]/2) - (PATH[i]%2) ) * ( (PATH[m]/2) - (PATH[m]%2) ) * PHONON_KERNEL[i-m]);	
 }
...
snprintf(FILE_NAME,2048+1024,"%d_Bloch_Path_Integral_t=%.1fps_CutOff=%.5f_Nc=%i_DETUNING=%.6f_OmL=%.5f_%.0fK.dat",100*100*hour+100*min+sec,m*delta_t*0.001,CUT_OFF_DENSITY/GOmL,Nc,DELTA/GOmL,GOmL,TEMPERATUR); 
FILE *f_dat; f_dat = fopen(FILE_NAME,"w");
for(k=0;k<21+Dt;k++)
{
fprintf(f_dat,"%.5f \t",delta_t*k*0.001);
fprintf(f_dat,"%.5f \t %.10f \t",DYNAMICS_01[k],DYNAMICS_11[k]);
fprintf(f_dat,"%.5f \t %.10f \t",sin(PULSE_AREA[k-1])*sin(PULSE_AREA[k-1]),0.5*sin(2.*PULSE_AREA[k-1]));
fprintf(f_dat,"%.5f \n",PULSE[k]/puls_max);
}
fclose(f_dat); 

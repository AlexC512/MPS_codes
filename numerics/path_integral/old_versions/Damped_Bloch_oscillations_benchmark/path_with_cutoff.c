...
for(i=0; i<N_b; i++)
      {
       ...
       kernel_temp += spec_fak * (2.*n_ph[i]+1.) * ( cos(c_q*float_q*  Mc     *delta_t) - cos(c_q*float_q*(Mc-1.)*delta_t)  ) ;  
       kernel_temp += spec_fak * (2.*n_ph[i]+1.) * ( cos(c_q*float_q*( tk-1. )*delta_t) - cos(c_q*float_q* 1.    *delta_t)  ) ;          
	   kernel_temp += -I * spec_fak              * ( sin(c_q*float_q*  Mc     *delta_t) - sin(c_q*float_q*(Mc-1.)*delta_t)  ) ;  
       kernel_temp += -I * spec_fak              * ( sin(c_q*float_q*( tk-1. )*delta_t) - sin(c_q*float_q* tk    *delta_t)  ) ;          
      }
...
for(i=0; i<N_b; i++)
      {
       float_q  = q_0 + delta_q * i;
       spec_fak = -1. * delta_q * (2./(2.*M_PI*M_PI*c_q*c_q)) * la_coup[i]  * conj(la_coup[i]) ;  
       kernel_temp += spec_fak * (2.*n_ph[i]+1.) * ( cos(c_q*float_q*  Mc     *delta_t) - cos(c_q*float_q*(Mc-1.)*delta_t)  ) ;  
       kernel_temp += spec_fak * (2.*n_ph[i]+1.) * ( cos(c_q*float_q*( tk-1. )*delta_t) - cos(c_q*float_q* 1.    *delta_t)  ) ;          
	   kernel_temp += -I * spec_fak              * ( sin(c_q*float_q*  Mc     *delta_t) - sin(c_q*float_q*(Mc-1.)*delta_t)  ) ;  
       kernel_temp += -I * spec_fak              * ( sin(c_q*float_q*( tk-1. )*delta_t) - sin(c_q*float_q* 1.   *delta_t)  ) ;          
      }


if (Dt>0) PHONON_KERNEL[28] -= kernel_temp;    

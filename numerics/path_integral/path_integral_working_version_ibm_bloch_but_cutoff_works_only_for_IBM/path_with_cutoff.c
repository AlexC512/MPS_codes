 ...
 ***************************************************/
 ...
 #define CUT_OFF_DENSITY ( 0.00000001 * 100.0 * 1.  )
 ...
 #define N_DELTA_T ( 200 )
 #define MEMORY_LENGTH ( 12 )
...
printf("PHONON_KERNEL[%i]=%.15f + I %.15f \n",p,creal(PHONON_KERNEL[p]),cimag(PHONON_KERNEL[p]) );
}
...
// ####################################################
// ####################################################
complex double kernel_temp = 0.;
double Mc=Nc-1; double tk = 30.;
complex double Snm = 1.;
int DEPTH = 0;

complex double cutoff_ADD[Nc];
for(l=1;l<Nc+1;l++)
{	
Dt=l;
Mc = Nc-1.; tk=Dt;
for(i=0; i<N_b; i++)
      {
       float_q  = q_0 + delta_q * i;
       spec_fak = -1. * delta_q * (1./(2.*M_PI*M_PI*c_q*c_q)) * la_coup[i]  * conj(la_coup[i]) ;  
       cutoff_ADD[l] += spec_fak * (2.*n_ph[i]+1.) * ( cos(c_q*float_q*  Mc     *delta_t) - cos(c_q*float_q*(Mc-1.)*delta_t)  ) ;  
       cutoff_ADD[l] += spec_fak * (2.*n_ph[i]+1.) * ( cos(c_q*float_q*( tk-1. )*delta_t) - cos(c_q*float_q* tk    *delta_t)  ) ;          
	   cutoff_ADD[l] += -I * spec_fak              * ( sin(c_q*float_q*  Mc     *delta_t) - sin(c_q*float_q*(Mc-1.)*delta_t)  ) ;  
       cutoff_ADD[l] += -I * spec_fak              * ( sin(c_q*float_q*( tk-1. )*delta_t) - sin(c_q*float_q* tk   *delta_t)  ) ;          
      }
}
// the following works just for the IBM ... 
/*
complex double kernel_temp = 0.;
Dt=1;
Mc = Nc-1.; tk=Dt;
for(i=0; i<N_b; i++)
      {
       float_q  = q_0 + delta_q * i;
       spec_fak = -1. * delta_q * (1./(2.*M_PI*M_PI*c_q*c_q)) * la_coup[i]  * conj(la_coup[i]) ;  
       kernel_temp1 += spec_fak * (2.*n_ph[i]+1.) * ( cos(c_q*float_q*  Mc     *delta_t) - cos(c_q*float_q*(Mc-1.)*delta_t)  ) ;  
       kernel_temp1 += spec_fak * (2.*n_ph[i]+1.) * ( cos(c_q*float_q*( tk-1. )*delta_t) - cos(c_q*float_q* tk    *delta_t)  ) ;          
	   kernel_temp1 += -I * spec_fak              * ( sin(c_q*float_q*  Mc     *delta_t) - sin(c_q*float_q*(Mc-1.)*delta_t)  ) ;  
       kernel_temp1 += -I * spec_fak              * ( sin(c_q*float_q*( tk-1. )*delta_t) - sin(c_q*float_q* tk   *delta_t)  ) ;          
      }
*/
...
printf("-------- DT=%i ----------------- Nc-2=%i \n",Dt,Nc-2);	
DEPTH = 0;
//printf("KERNEL_ADD %.10f+I%.10f \n",creal(kernel_temp),cimag(kernel_temp));
// we must correct for the cut-off in the longest applied memory kernel counting from zero 
// if (Dt>0) PHONON_KERNEL[Nc-2] -= kernel_temp;    // for the IBM cutoff 
/*
if (Dt>0) PHONON_KERNEL[Nc-2] -= cutoff_ADD[1];    // 10 
if (Dt>1) PHONON_KERNEL[Nc-3] -= cutoff_ADD[2];    // 9 
if (Dt>2) PHONON_KERNEL[Nc-4] -= cutoff_ADD[3];    // 8 
if (Dt>3) PHONON_KERNEL[Nc-5] -= cutoff_ADD[4];    // 7 
if (Dt>5) PHONON_KERNEL[Nc-6] -= cutoff_ADD[5];    // 6 
if (Dt>6) PHONON_KERNEL[Nc-7] -= cutoff_ADD[6];    // 5 
if (Dt>7) PHONON_KERNEL[Nc-8] -= cutoff_ADD[7];    // 4
if (Dt>8) PHONON_KERNEL[Nc-9] -= cutoff_ADD[8];    // 3 
if (Dt>9) PHONON_KERNEL[Nc-10] -= cutoff_ADD[9];   // 2  
if (Dt>10) PHONON_KERNEL[Nc-11] -= cutoff_ADD[10];  // 1   
if (Dt>11) PHONON_KERNEL[Nc-12] -= cutoff_ADD[11]; // 0    
*/
...
printf("[%i]:t=%.5fps: IM[01]=%.10f -- IBM  =%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(RESULT[1]),creal(IBM[p]),creal(IBM[p])-cimag(RESULT[1]) );
printf("[%i]:t=%.5fps: RE[01]=%.10f -- IBM  =%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(RESULT[1]),cimag(IBM[p]),cimag(IBM[p])-creal(RESULT[1]) );
...
printf("[%i]:t=%.5fps: RE[01]=%.10f -- IBM  =%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(RESULT[1]),cimag(IBM[p]),cimag(IBM[p])-creal(RESULT[1]) );
printf("[%i]:t=%.5fps: ADM_11=%.10f -- EXACT=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(RESULT[3]),sin(PULSE_AREA[m-1])*sin(PULSE_AREA[m-1]),creal(RESULT[3])-sin(PULSE_AREA[m-1])*sin(PULSE_AREA[m-1]) );
printf("[%i]:t=%.5fps: PULSE =%.10f -- MEM_DEPTH=%i -- INC_RE[01]=%.10f \n\n",m,m*delta_t*0.001,creal(PULSE[m-1]),DEPTH,10000000.*(creal(RESULT[1])-RE_POL[m-1]));
...
snprintf(FILE_NAME,2048+1024,"%d_Bloch_Phonon_t=%.1fps_PHON_FAK%.02f_CutOff=%.5f_Nc=%i_DET=%.2f_GOmL=%.5f_OmL=%.5f_%.0fK.dat",100*100*hour+100*min+sec,m*delta_t*0.001,PHON_FAK,CUT_OFF_DENSITY/(GOmL+OmL),Nc,DELTA/(GOmL+OmL),GOmL,OmL,TEMPERATUR); 

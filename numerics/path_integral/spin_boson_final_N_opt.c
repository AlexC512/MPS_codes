 ...
 ***************************************************/
 ...
 #define CUT_OFF_DENSITY ( 0.00000001 * 10.10 * 0.  )
 ...
 #define N_DELTA_T ( 4 )
 #define MEMORY_LENGTH ( 7 ) // Memory length 4 means 0,1,2,3 time steps with 0 initial state, so 3 time steps 
...
PHONON_KERNEL[0] -= SUM_KERNEL;
SUM_KERNEL = 0.;

for(p=0;p<Nc+5;p++)
{
if (p>Nc-1) PHONON_KERNEL[p] = 0.;  
SUM_KERNEL += PHONON_KERNEL[p];    
printf("PHONON_KERNEL[%i]=%.20f + I %.20f -- SUM_KERNEL=%.20f+I%.20f \n",p,creal(PHONON_KERNEL[p]),cimag(PHONON_KERNEL[p]),creal(SUM_KERNEL),cimag(SUM_KERNEL) );
}
//*/
...
void PRINT_COMPLEX(complex double toprint)
{
printf("%.10f+I%.10f \n",creal(toprint),cimag(toprint));   
return;
}
...
DYNAMICS_01[0]=cimag(PATH_INIT_2[1]);
DYNAMICS_11[0]=creal(PATH_INIT_2[2]);
RE_POL[0]    =creal(PATH_INIT_2[1]);
...
{	
 ...
 if (HELPER==0) { for(m=0;m<Nc-1;m++) PATH[m]=  path(k,m); }
 else           { for(m=0;m<Nc-1;m++) PATH[m]=path_2(k,m); }
 ...
 for(i=1;i<=p;i++) 
   {    
	...
	for(m=0;m<i;m++)  Snm *= cexp( ((PATH[i]/2) - (PATH[i]%2)) * ((PATH[i-m]/2)*PHONON_KERNEL[m]-(PATH[i-m]%2)*conj(PHONON_KERNEL[m])) ); 
   } 
 
 if ( ( cabs(test) > CUT_OFF_DENSITY ) )   
    {
         RESULT[l] += test * Snm;
	     /*Snm =  cexp( ((PATH[0]/2) - (PATH[0]%2)) * ((PATH[0]/2)*PHONON_KERNEL[0]-(PATH[0]%2)*conj(PHONON_KERNEL[0])) )
		       *cexp( ((PATH[1]/2) - (PATH[1]%2)) * ((PATH[0]/2)*PHONON_KERNEL[1]-(PATH[0]%2)*conj(PHONON_KERNEL[1])) ) 
			   *cexp( ((PATH[2]/2) - (PATH[2]%2)) * ((PATH[0]/2)*PHONON_KERNEL[2]-(PATH[0]%2)*conj(PHONON_KERNEL[2])) ) 
			   *cexp( ((PATH[3]/2) - (PATH[3]%2)) * ((PATH[0]/2)*PHONON_KERNEL[3]-(PATH[0]%2)*conj(PHONON_KERNEL[3])) ) 
			   *cexp( ((PATH[4]/2) - (PATH[4]%2)) * ((PATH[0]/2)*PHONON_KERNEL[4]-(PATH[0]%2)*conj(PHONON_KERNEL[4])) ) ; */
		 Snm = 1.;	   
   		 for(m=0;m<Nc;m++) Snm *= cexp( ((PATH[m]/2) - (PATH[m]%2)) * ((PATH[0]/2)*PHONON_KERNEL[m]-(PATH[0]%2)*conj(PHONON_KERNEL[m])) ) ;	   			   
 		 if (HELPER == 0) {  for(m=0;m<Nc;m++) path_2(NON_NEGLIGIBLE,m)=PATH[m];                    							 
                             if (p==Nc-1) 
							    { 
								INITIAL_STATES_2[NON_NEGLIGIBLE]  = Snm * M_Om((PATH[1]/2),(PATH[0]/2),0)*PATH_INIT[PATH[0]]*conj(M_Om((PATH[1]%2),(PATH[0]%2),0));
								}
                          }
		 else             { for(m=0;m<Nc;m++) {  path(NON_NEGLIGIBLE,m)=PATH[m];     }
		                    if (p==Nc-1) 
							   { 
  							    INITIAL_STATES[NON_NEGLIGIBLE]  = Snm * M_Om((PATH[1]/2),(PATH[0]/2),0)*PATH_INIT[PATH[0]]*conj(M_Om((PATH[1]%2),(PATH[0]%2),0));
 								}                                                       
						  }
         NON_NEGLIGIBLE++;
     }
...
printf("[%i]:t=%.5fps: RE[01]=%.10f -- DIFF =%.10f  \n",m,m*delta_t*0.001,creal(RESULT[1]),creal(RESULT[1])-RE_POL[p-1] );
...
printf("[%i]:t=%.5fps: IM[01]=%.10f -IM[IBM]=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(RESULT[1]),creal(IBM[m-1]),creal(IBM[m-1])-cimag(RESULT[1]) );
...
int *MAP = calloc( pow4[MAX_POWER], sizeof(int) );
int tempMAP,tempMAP1;
...
{	
 // first write path array   
 if (HELPER==0) { for(m=0;m<Nc-1;m++) PATH[m]=  path(k,m+1); test=INITIAL_STATES[k]; tempMAP += PATH[m]*pow4[m]; }
 else           { for(m=0;m<Nc-1;m++) PATH[m]=path_2(k,m+1); test=INITIAL_STATES_2[k]; tempMAP += PATH[m]*pow4[m]; }
 PATH[Nc-1] = l ;  // PRINT_PATH(PATH,Nc);
 tempMAP1 = PATH[Nc-1] * pow4[Nc-1]; 
 Snm = 1.;
 for(i=1;i<=Nc-1;i++) 
 {    
    test *= M_Om((PATH[i]/2),(PATH[i-1]/2),0) * conj(M_Om((PATH[i]%2),(PATH[i-1]%2),0));
 ...
 if ( ( cabs(test) > CUT_OFF_DENSITY ) )   
    {
         ...
         if (HELPER == 0) { 
		                    ...
		                    if ( CHECK==1 ) INITIAL_STATES_2[m] += Snm * M_Om((PATH[1]/2),(PATH[0]/2),0) * INITIAL_STATES[k] * conj(M_Om((PATH[1]%2),(PATH[0]%2),0));
							else 
							{
							for(m=0;m<Nc;m++) path_2(NON_NEGLIGIBLE,m)=PATH[m];  
                            INITIAL_STATES_2[NON_NEGLIGIBLE] = Snm * M_Om((PATH[1]/2),(PATH[0]/2),0) * INITIAL_STATES[k] * conj(M_Om((PATH[1]%2),(PATH[0]%2),0));
		                    NON_NEGLIGIBLE++;
	   						}
                          }
		 else             { 
		                    ...
		                    if ( CHECK==1 ) INITIAL_STATES[m] += Snm * M_Om((PATH[1]/2),(PATH[0]/2),0) * INITIAL_STATES_2[k] * conj(M_Om((PATH[1]%2),(PATH[0]%2),0));
							else 
							{
							for(m=0;m<Nc;m++) path(NON_NEGLIGIBLE,m)=PATH[m];  
                            INITIAL_STATES[NON_NEGLIGIBLE] = Snm * M_Om((PATH[1]/2),(PATH[0]/2),0) * INITIAL_STATES_2[k] * conj(M_Om((PATH[1]%2),(PATH[0]%2),0));
							NON_NEGLIGIBLE++;
	   						}	
    	                  }         
	}
...
printf("[%i]:t=%.5fps: ADM_11=%.10f -- BLOCH=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,creal(RESULT[3]),sin(PULSE_AREA*(m-1))*sin(PULSE_AREA*(m-1)),creal(RESULT[3])-sin(PULSE_AREA*(m-1))*sin(PULSE_AREA*(m-1)) );
printf("[%i]:t=%.5fps: IM[01]=%.10f -IM[IBM]=%.10f -- DIFF=%.10f \n",m,m*delta_t*0.001,cimag(RESULT[1]),creal(IBM[m-1]),creal(IBM[m-1])-cimag(RESULT[1]) );
DYNAMICS_01[m-1]=cimag(RESULT[1]);
DYNAMICS_11[m-1]=creal(RESULT[3]);
     RE_POL[m-1]=creal(RESULT[1]);
printf("RELEVANT STATES %i \n",NON_NEGLIGIBLE);
}
} // Dt loop
...
free(INITIAL_STATES);
free(INITIAL_STATES_2);
free(REL_STATES_INDEX);
free(REL_STATES_INDEX_2);

snprintf(FILE_NAME,2048+1024,"%d_Changed_Bloch_Phonon_w_MEM_CUT_t=%.1fps_PHON_FAK%.02f_CutOff=%.5f_Nc=%i_DET=%.2f_GOmL=%.5f_OmL=%.5f_%.0fK.dat",100*100*hour+100*min+sec,m*delta_t*0.001,PHON_FAK,CUT_OFF_DENSITY/(GOmL+OmL),Nc,DELTA/(GOmL+OmL),GOmL,OmL,TEMPERATUR); 

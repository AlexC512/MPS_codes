...
void MPO_SETUP(ITensor& U_evo, const std::vector<Index>& bin,
               int tls, int l_past, int l_middle, int r_middle, int l_now, int Nbin,
              ...
              )
{
    ...
    Complex   gam_ml_fb =  -Cplx_i*Gamma_m*sqrt(dt)*exp(-Cplx_i*3.14159265359*phi_ml); 
    Complex c_gam_ml_fb =  -Cplx_i*Gamma_m*sqrt(dt)*exp( Cplx_i*3.14159265359*phi_ml);  
    Complex   gam_mr_fb =  -Cplx_i*Gamma_m*sqrt(dt)*exp(-Cplx_i*3.14159265359*phi_mr); 
    Complex c_gam_mr_fb =  -Cplx_i*Gamma_m*sqrt(dt)*exp( Cplx_i*3.14159265359*phi_mr);  
    ...
    // -------------------------------- middle atom -----------------------------------------------------------------
    // middle atom interation hamiltonian ... photons in time bins are created ... middle atom has no separate feedback int
    H_dis_ml.set(bin[tls](8),prime(bin[tls](6)),bin[l_middle](phot),prime(bin[l_middle](phot+1)), gam_ml_fb*sqrt_phot);
    H_dis_ml.set(bin[tls](7),prime(bin[tls](5)),bin[l_middle](phot),prime(bin[l_middle](phot+1)), gam_ml_fb*sqrt_phot);        
    H_dis_ml.set(bin[tls](4),prime(bin[tls](2)),bin[l_middle](phot),prime(bin[l_middle](phot+1)), gam_ml_fb*sqrt_phot);        
    H_dis_ml.set(bin[tls](3),prime(bin[tls](1)),bin[l_middle](phot),prime(bin[l_middle](phot+1)), gam_ml_fb*sqrt_phot);        
    // interaction with left moving field
    H_dis_ml.set(bin[tls](6),prime(bin[tls](8)),bin[l_middle](phot+1),prime(bin[l_middle](phot)), c_gam_ml_fb*sqrt_phot);
    H_dis_ml.set(bin[tls](5),prime(bin[tls](7)),bin[l_middle](phot+1),prime(bin[l_middle](phot)), c_gam_ml_fb*sqrt_phot);        
...
// ###########################################################################################################
// ###########################################################################################################
void SWAP_LEFT_MIDDLE_FORWARD(MPS& psi, const std::vector<Index>& bin, int from, int to, double cutoff)
{
    ITensor SWAP,U,S,V;
    Index iLeft=findIndex(psi(from),"left");
    ...
    {    
    SWAP = psi(k)*psi(k+1);
    U=ITensor(iLeft,commonIndex(psi(k+1),psi(k+2)));
    svd(SWAP,U,S,V,{"Cutoff=",cutoff});
    psi.setA(k,V); 
    psi.setA(k+1,U*S);
    }
...
// ###########################################################################################################
// ###########################################################################################################
void SWAP_LEFT_MIDDLE_BACKWARD(MPS& psi, const std::vector<Index>& bin, int from, int to, double cutoff)
{
    ITensor SWAP,U,S,V;
    Index iLeft=findIndex(psi(from),"left");
...
int l_fb  = input.getInt("left_fb");
int r_fb  = input.getInt("right_fb");
if ( r_fb>l_fb ) { printf("ATTENTION: middle-right feedback is larger than left-middle!!!\n"); return 0;}
int Nfb = 2*(l_fb+r_fb); // size of array is twice because of left and right moving photons and one feedback loop 
...
int par_max = input.getInt("par_max");
...
if (Nfb>4) snprintf(FILE_NAME,2048+1024,"%02d_%02d_%02d_%02d_%02d_%02d_N3_PAR_PHASE_l_fb%i_r_fb%i_N%i_par_max_%i_init1_%.2f_init2_%.2f_init3_%.2f_dt_%.4f_CutOff_%.4f.dat",year,month+1,day,hour,minute,second,l_fb,r_fb,t_end,par_max,pop1,pop2,pop3,dt,cutoff*1000000.);
else snprintf(FILE_NAME,2048+1024,"%02d_%02d_%02d_%02d_%02d_%02d_N3_OCCUPATIONS_l_fb%i_r_fb%i_N%i_sum_pop_%.2f_p_l_%.2f_p_ml_%.2f_p_mr_%.2f_p_r_%.2f_dt_%.4f_CutOff_%.4f.dat",year,month+1,day,hour,minute,second,l_fb,r_fb,t_end,pop3+pop2+pop1,phi_l,phi_ml,phi_mr,phi_r,dt,cutoff*1000000.);
...
for(int j = 1; j <= ttotal; ++j){ binlink.at(j) = Index(1,"Link"); 
                                  if (j%2==1) bin.at(j)     = Index(Nbin,"left");
                                  else        bin.at(j)     = Index(Nbin,"right");         
                                                                                   }
tls=0; bin.at(tls) = Index(8,"sys"); 
...
// for asymmetric feedback define left distance, right distance bin .... TODO
    ...
    sys_at=Nfb+1;
int l_past= sys_at-Nfb; // Nfb=2(r_fb+l_fb);
int r_past= sys_at-Nfb+1;
int l_now = Nfb+1; 
int r_now = Nfb+2;
int l_middle = sys_at-2*l_fb   ;
int r_middle = sys_at-2*r_fb +1;    
...
// --------------------- START EVOLUTION  -------------------------------------------
// ----------------------------------------------------------------------------------
if (Nfb>4) // so includes every feedback with l_fb>1 and r_fb >=1
{

double pop_min=1.0; int parmin;
double pop_max=0.0; int parmax;
double prev1,prev2,prev3;

for (int par=0.; par<=par_max; par++)
{
for(int j = 1; j <= ttotal; ++j){             binlink.at(j) = Index(1,"Link");  
                                  if (j%2==1) bin.at(j)     = Index(Nbin,"left");
                                  else        bin.at(j)     = Index(Nbin,"right");         
...
l_now = Nfb+1; 
r_now = Nfb+2;
l_middle = sys_at-2*l_fb   ;
r_middle = sys_at-2*r_fb +1;    
...
phi_l=par*2./par_max; 
// one phase determines all
phi_r  = phi_l; 
phi_ml = l_fb*phi_l/(l_fb+r_fb); 
phi_mr = r_fb*phi_l/(l_fb+r_fb);
MPO_SETUP(U_evo,bin,tls,l_past,l_middle,r_middle,l_now,Nbin,Gamma_l,Gamma_m,Gamma_r,phi_l,phi_ml,phi_mr,phi_r,dt);
...
{   
    ...
    prev1=pop1; prev2=pop2; prev3=pop3;
    // --- End - Status and File output -----------------------------------------------------------------------------------
    // -------------- PREPARE MPS TO APPLY MPO ------------------------------------------
    SWAP_LEFT_MIDDLE_FORWARD(psi,bin,l_middle,r_middle-1,cutoff);
    ...
    // ----------- SWAP BACK START -----------------------------
    SWAP_LEFT_BACKWARD(psi,bin,sys_at-4,r_middle-3,cutoff);
    SWAP_LEFT_MIDDLE_BACKWARD(psi,bin,r_middle-3,l_middle-2,cutoff);    
...
if (pop3>pop_max) {pop_max = pop3; parmax=par;} if ((pop3>0.0000000001) && (pop3<pop_min)) {pop_min=pop3; parmin=par;}
if (pop2>pop_max) {pop_max = pop2; parmax=par;} if ((pop2>0.0000000001) && (pop2<pop_min)) {pop_min=pop2; parmin=par;}
if (pop1>pop_max) {pop_max = pop1; parmax=par;} if ((pop1>0.0000000001) && (pop1<pop_min)) {pop_min=pop1; parmin=par;}

printf("(%.2f,%.2f,%.2f,%.2f)",phi_l,phi_ml,phi_mr,phi_r);
printf("pre_l=%.10f -- pre_m=%.10f -- pre_r=%.10f \n",prev3,prev2,prev1); 
printf("(%.2f,%.2f,%.2f,%.2f)",phi_l,phi_ml,phi_mr,phi_r);
printf("pop_l=%.10f -- pop_m=%.10f -- pop_r=%.10f --",pop3,pop2,pop1); 
printf("l_out=%.10f -- r_out=%.10f -- sum=%.10f",sum_l,sum_r,sum_r+sum_l+pop3+pop2+pop1); 
printf("\n");
fprintf(file,"## %.2f \t %.2f \t %.2f \t %.2f \n",phi_l,phi_ml,phi_mr,phi_r);
fprintf(file,"## %i \t %.10f \t %.10f \t %.10f \n",par,prev3,prev2,prev1);
fprintf(file,"%i \t %.10f \t %.10f \t %.10f \n",par,pop3,pop2,pop1);

} // end par

printf("## pop_min(%i)=%.10f -- pop_max(%i)=%.10f \n",parmin,pop_min,parmax,pop_max);
fprintf(file,"## pop_min(%i)=%.10f -- pop_max(%i)=%.10f \n",parmin,pop_min,parmax,pop_max);
...
MPO_NO_FB_SETUP(U_evo,bin,tls,l_now,Nbin,Gamma_l,Gamma_m,Gamma_r,dt,phi_l,phi_ml,phi_r);

...
if (Nfb>0)  temp_delta = temp_delta * delta(bin[1],prime(bin[1])); 
...
for(j = Nfb+1; j<=ttotal;j++)
{
 ...
 if (Nfb>1) { psi.setA(j,Vp  ); psi.setA(j-1,U*S);} 
 else       { psi.setA(j,Vp*S); psi.setA(j-1,U  );}
 ...
 {    
 iCB=findIndex(psi(j+1),"bath");
 iLink=commonIndex(psi(j),psi(j-1));
 temp=noPrime(U_evo * psi(j)*psi(j+1)); 
 U=ITensor(iCB,iLink);
 svd(temp,U,S,Vp);

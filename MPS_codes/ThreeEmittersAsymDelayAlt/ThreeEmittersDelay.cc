...
int ALTERNATIVE_SWAP = input.getInt("SWAP_ALTERNATIVE");
...
snprintf(FILE_NAME,2048+1024,"%02d_%02d_%02d_%02d_%02d_%02d_N3_OCCUPATIONS_l_fb%i_r_fb%i_N%i_sum_pop_%.2f_p_l_%.2f_p_ml_%.2f_p_mr_%.2f_p_r_%.2f_dt_%.4f_CutOff_%.4f.dat",year,month+1,day,hour,minute,second,l_fb,r_fb,t_end,pop3+pop2+pop1,phi_l,phi_ml,phi_mr,phi_r,dt,cutoff*1000000.);
file = fopen(FILE_NAME,"w");

snprintf(FILE_NAME,2048+1024,"%02d_%02d_%02d_%02d_%02d_%02d_N3_PROBABILITY_l_fb%i_r_fb%i_N%i_sum_pop_%.2f_p_l_%.2f_p_ml_%.2f_p_mr_%.2f_p_r_%.2f_dt_%.4f_CutOff_%.4f.dat",year,month+1,day,hour,minute,second,l_fb,r_fb,t_end,pop3+pop2+pop1,phi_l,phi_ml,phi_mr,phi_r,dt,cutoff*1000000.);
...
{   
    ...
    if ( ALTERNATIVE_SWAP == 1)
    {
    SWAP_FORWARD(psi,bin,l_middle,sys_at-1,cutoff);
    SWAP_FORWARD(psi,bin,r_middle-1,sys_at-1,cutoff);
    SWAP_FORWARD(psi,bin,l_past,sys_at-1,cutoff);
    SWAP_FORWARD(psi,bin,r_past-1,sys_at-1,cutoff); 
    }
    else 
    {    
    ...
    if ( ALTERNATIVE_SWAP == 1)
    {
    SWAP_BACKWARD(psi,bin,sys_at-4,l_middle-2,cutoff);//Print(psi(l_middle-2));
    SWAP_BACKWARD(psi,bin,sys_at-3,r_middle-2,cutoff);

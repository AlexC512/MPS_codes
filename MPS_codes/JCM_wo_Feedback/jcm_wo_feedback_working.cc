...
{    
 ...
 H_sys.set(bin[cav](phot  ),prime(bin[cav](phot+1)),-Cplx_i*dt*E_cav*sqrt_phot);
 H_sys.set(bin[cav](phot+1),prime(bin[cav](phot  )),-Cplx_i*dt*E_cav*sqrt_phot);
}
...
{    
 sqrt_phot=sqrt(1.*phot);   
 H_jcm.set(bin[tls](1),prime(bin[tls](2)),bin[cav](phot+1),prime(bin[cav](phot)  ),-Cplx_i*dt*gcav*sqrt_phot);
 H_jcm.set(bin[tls](2),prime(bin[tls](1)),bin[cav](phot)  ,prime(bin[cav](phot+1)),-Cplx_i*dt*gcav*sqrt_phot);    
} // */
...
{    
 ...
 {    
  ...
  H_dis.set(bin[cav](phot+1),prime(bin[cav](phot  )),bin[1](bphot  ),prime(bin[1](bphot+1)),-Cplx_i*sqrt(dt)*Gamma*sqrt_phot);
  H_dis.set(bin[cav](phot  ),prime(bin[cav](phot+1)),bin[1](bphot+1),prime(bin[1](bphot  )),-Cplx_i*sqrt(dt)*Gamma*sqrt_phot);
 }
}//
...
temp = temp * delta(bin[cav],prime(bin[cav])); 
temp = temp * delta(bin[1],prime(bin[1])); 

auto U_evo = temp + H_int_1 + H_int_2 + H_int_3 + H_int_4 + H_int_5 
...
for(j = 1; j<=ttotal;j++)
{
 if( j%SHOW_EVERY_STEP == 0 ) printf("Step: %i of  %i -- cav=%.10f -- em=%.10f -- bath=%.10f -- sum_bath=%.10f -- bath_coh=%.10f norm=%.10f \n",j,t_end,cavity_pop,emitter_pop,bath_pop,sum_bath,bath_coh,mps_norm );    
 fprintf(f_dat,"%.10f \t %.10f \t %.10f \t %.10f \t %.10f \n",j*dt,cavity_pop,emitter_pop,bath_coh.real(),bath_pop); 
 // fuer psi ####################################################
 temp=noPrime(U_evo * psi.A(j) * psi.A(j+1)); //PrintData(temp); 
...
// PrintData(psi(1));PrintData(psi(2));PrintData(psi(3));

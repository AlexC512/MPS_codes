...
double pop_min=1.0; int kmin,lmin;
double pop_max=0.0; int kmax,lmax;
double loc_max=0.0;
double loc_min=0.0;
...
// setup MPO    
if (Nmax>1)
{phi_l=k*2./Nmax; // phase difference phi_l-phi_m
phi_m=0.; // only relative phases matter
phi_r=l*2./Nmax; // phase difference phi_r-phi_m
}
...
if (pop3>pop_max) {pop_max = pop3; kmax=k; lmax=l;} if (pop3<pop_min) {pop_min=pop3; kmin=k; lmin=l;}
if (pop2>pop_max) {pop_max = pop2; kmax=k; lmax=l;} if (pop2<pop_min) {pop_min=pop2; kmin=k; lmin=l;}
if (pop1>pop_max) {pop_max = pop1; kmax=k; lmax=l;} if (pop1<pop_min) {pop_min=pop1; kmin=k; lmin=l;}

if (pop3>loc_max) {loc_max = pop3; } if (pop3<loc_min) {loc_min=pop3; }
if (pop2>loc_max) {loc_max = pop2; } if (pop2<loc_min) {loc_min=pop2; }
if (pop1>loc_max) {loc_max = pop1; } if (pop1<loc_min) {loc_min=pop1; }


printf("(%.2f,%.2f,%.2f)",phi_l,phi_m,phi_r);
printf("pop_l=%.10f -- pop_m=%.10f -- pop_r=%.10f --",pop3,pop2,pop1); 
printf("loc_max=%.10f -- loc_min=%.10f -- sum=%.10f",loc_max,loc_min,sum_r+sum_l+pop3+pop2+pop1); 
printf("\n");
fprintf(file,"## %.2f \t %.2f \t %.2f \n",phi_l,phi_m,phi_r);
fprintf(file,"%i \t %.10f \t %.10f \t %.10f \t %.10f \t %.10f \n",counter,loc_min,loc_max,pop3,pop2,pop1);
counter++;
}
}

printf("## pop_min(%i,%i,%i)=%.10f -- pop_max(%i,%i,%i)=%.10f \n",kmin,0,lmin,pop_min,kmax,0,lmax,pop_max);
fprintf(file,"## pop_min(%i,%i,%i)=%.10f -- pop_max(%i,%i,%i)=%.10f \n",kmin,0,lmin,pop_min,kmax,0,lmax,pop_max);
